...
do
	...
	# reference bandwidth
	mkdir voip-poo-ts-05s-2_5s-$lossExp
	cd voip-poo-ts-05s-2_5s-$lossExp
	nsttn ../fhb-1mb-wlen-50-ts-02s-1s.tcl $simTime 0 Voip POO $lossExp
	gawk -v simTime=$simTime -v warmUpTime=$warmUpTime -v capacity=$capacity -v lossExp=$lossExp -f  ../calcStdStats.awk qmon >>  ../voip-poo-ts-05s-2_5s-results.txt
	cd ..
...
# 	# plug-in bandwidth
# 	mkdir voip-$lossExp
# 	cd voip-poo-ts-05s-2_5s-hhat-$lossExp
