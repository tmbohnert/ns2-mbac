...
set tsm 5
set wlen 20
$dsQueue addAdcEntry $dscp0 SKDE ACCEPT $bnTxRate [expr $bufferSize*$packetSize] $txRate $pLoss TRE $hRef $j IEDE TRE $tau $tsm $wlen TRDE END
#$dsQueue addAdcEntry $dscp0 SSUM  [expr $bnTxRate] $txRateVideo
$dsQueue printAdcSetup

# Schedule events
puts "GENERATE RANDOM VARIABLES"
# (seed = 1 creates pseudo random variables, 0 non-deterministic but not necessarily independent variables!)
set rgFi [new RNG]
$rgFi seed 0
set rgFl [new RNG]
$rgFl seed 0
...
proc finish {} {
	global ns qmon simIdx
	$ns flush-trace
	exit 0
}

proc queue-monitoring {} {
	global ns cr dnode simTime qmon warmUpTime lossExp
